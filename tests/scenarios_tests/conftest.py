...
@pytest.fixture([
    np.array([3000, 5000, 15000]),    # Uniform Distribution : Ensures a balanced number of samples across different DOA combinations.
    np.array([5000, 5000, 5000]),     # Flat Distribution : Allocates the same number of samples irrespective of `min(nb_ref, nb_pred)`.
    np.array([1000, 3000, 31000]),    # Exponential Distribution : Significantly increases the number of samples for higher values of `min(nb_ref, nb_pred)`.
    np.array([2600, 5000, 17000]),    # Slightly Increasing Distribution : Gradually increases the number of samples as `min(nb_ref, nb_pred)` increases.
    np.array([6300, 4000, 1500]),     # Reverse Exponential Distribution : Allocates more samples to lower `min` values and fewer to higher ones.
    np.array([2000, 7000, 14000]),    # Custom Mixed Emphasis 1 : Significantly increases the number of samples for higher values of `min(nb_ref, nb_pred)`.
    np.array([2500, 8000, 8500])      # Custom Mixed Emphasis 2 : Combines elements of uniform and skewed distributions for balanced yet focused sampling.
