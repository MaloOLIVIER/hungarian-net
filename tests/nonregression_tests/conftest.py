...
def test_non_regression_train_hnet(mocker):
    ...
    assert os.path.exists(reference_model_path), f"Reference model not found at {reference_model_path}"
    
    # Load the reference model's state dict
    reference_state_dict = torch.load(reference_model_path, map_location=torch.device('cpu'))
    ...
    for key in reference_state_dict:
        ...
        assert torch.allclose(ref_tensor, trained_tensor, atol=1e-5), f"Mismatch found in layer: {key}"
    
    # Ensure no extra keys in the trained model
    assert len(trained_state_dict) == len(reference_state_dict), "Trained model has unexpected additional layers."
